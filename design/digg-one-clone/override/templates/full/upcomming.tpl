...
{elseif $node.node_id|eq( ezini( 'DiggSettings', 'AllNodeID', 'digg.ini' ) )}
{set $parent_node_id=ezini( 'DiggSettings', 'CategoryNodeID', 'digg.ini' )}
...
{* <div class="attribute-header">
	...
	<div class="content-view-children float-break">
    	  <div class="direction-control">Direction: {*<a href={concat($node.url_alias,'/(dir)/1')|ezurl}>down</a> and <a href={concat($node.url_alias,'/(dir)/0')|ezurl}>up</a>*}<a href={concat($node.url_alias,'/(dir)/1')|ezurl}>upcoming</a> and <a href={concat($node.url_alias,'/(dir)/0')|ezurl}>top</a> or go back to read the <a href={"/"|ezurl()}>home page</a></div>
    ...
    {foreach $childrenNodes as $index => $child}
         ...
         {if $index|le($page_limit_navigator)}{node_view_gui view=line content_node=$child[1] current_node=$node}{/if}
	 {/cache-block}
         {delimiter}
    ...
    <div class="notice-more">
     Looking for more? Try submitting a new stories to DiggClone's <a href="/c">categories</a> section which then will apear in the <a href="/all/(dir)/1">upcoming</a> section for voting to begin!
    </div>
