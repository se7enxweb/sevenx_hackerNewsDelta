...
<div class="content-view-line">
    ...
    {if $node.data_map.location.has_content}
        <div class="attribute-link">
            <p><a href="{$node.data_map.location.content}"{if $node.data_map.open_in_new_window.content|eq(1)} target="_blank"{/if}>{if $node.data_map.location.data_text|count|gt( 0 )}{$node.data_map.location.data_text|wash}{else}{$node.data_map.location.content|wash}{/if}</a></p>
        </div>
    {/if}
