...
{def $total_count=fetch( 'contextual', 'collected_digg_info_count_list', hash( 'object_attribute_id', $node.data_map.diggs.id ) )[0]}
...
<div class="content-view-full">
    ...
    <ul class="news-digg">
      <li class="digg-count shade-5" id="main0"><a href={concat($node.url_alias, "/")|ezurl()} id="diggs0">{cache-block keys=$total_count}{if $total_count|gt(0)}{$total_count}{else}0{/if}{/cache-block} Votes</a></li>
      <li class="digg-it" id="diglink0" onClick="document.getElementById('ActionCollectInformation').click(); return false;"><a href="https://clone.digg.one/notice">Vote for it</a></li>
      <li class="undigg-it" id="undiglink0" onClick="document.getElementsByName('ContentObjectAttribute_data_option_value_{$node.data_map.diggs.contentclassattribute_id}')[1].checked = true;document.getElementById('ActionCollectInformation').click(); return false;"><a href="https://digg.one/notice">UnVote</a></li>
    </ul>
...
%m/%d %h:%i %a' )} {*5 hours 33 minutes ago*} (<a href={$node.data_map.link.content|ezurl()} class="simple tight" title="{$node.data_map.title.content|wash}">via <a href={$node.data_map.link.content}>{$node.data_map.link.content}...</a>)</p>
     ...
     <div class="news-details">
       ...
       <span class="tool">category: <a href={$node.parent.url_alias|ezurl}>{$node.parent.name|wash}</a></span> | <a href={concat("/spy/collectionlist/", $node.object.id)|ezurl} class="">{"story digg history"}</a>
     </div>
...
-->
                ...
                {foreach $contentobject_attribute.content.option_list as $option}
		         ...
		         $option_value=false()}
                    ...
                    {/if}
		    {if $option.value|eq( 'digg' )}
		       {set $option_value='Vote'}
		    {/if}
		    {if $option.value|eq( 'undigg' )}
		       {set $option_value='Downvote'}
		    {/if}
	...
	{*
        ...
        {'%count total votes'|i18n( 'design/ezwebin/collectedinfo/poll' ,,
                                     hash( '%count', fetch( contextual, collected_digg_info_count, hash( object_id, $object.id ) ) ) )}
