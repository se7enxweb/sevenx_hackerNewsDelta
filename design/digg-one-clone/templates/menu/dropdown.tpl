...
{def $root_node_id=ezini( 'DiggSettings', 'MenuRootNodeID', 'digg.ini' )
     ...
     $top_level_exclude_node = array( 264, 381, 283, 267, 392 )
     $level_2_limit = 7
...
{else}
<input type="hidden" name="ClassID" value="54" />
{/if}
<input type="submit" name="NewButton" value="Submit" id="btn-submit" style="display:none"/>
</form>

<form name="create-image" method="post" action={'content/action'|ezurl}>
<input type="hidden" name="ContentNodeID" value="{$current_node_id}" />
<input type="hidden" name="ContentObjectID" value="{$current_node.contentobject_id}" />
<input type="hidden" name="NodeID" value="{$current_node_id}" />
{if $current_node.node_id|eq( 828 )}
...
{/if}
<input type="submit" name="NewButton" value="Submit" id="btn-image-submit" style="display:none"/>
</form>

<form name="create-youtube" method="post" action={'content/action'|ezurl}>
<input type="hidden" name="ContentNodeID" value="{$current_node_id}" />
<input type="hidden" name="ContentObjectID" value="{$current_node.contentobject_id}" />
<input type="hidden" name="NodeID" value="{$current_node_id}" />
{if $current_node.node_id|eq( 828 )}
...
<input type="hidden" name="ClassID" value="47" />
{/if}
<input type="submit" name="NewButton" value="Submit" id="btn-youtube-submit" style="display:none"/>
</form>
...
{if $top_level_exclude_node|contains( $item.node_id )}{continue}{/if}
...
{if or( $item.data_map.location.content|ne('/submit'), $item.data_map.location.content|eq('/content/view/sitemap/2'), $item.data_map.location.content|eq('/content/search/full/2'))}

{* master submit menu item *}
{run-once}
<a
  {if eq( $ui_context, 'browse' )}
href={concat("content/browse/", $item.node_id)|ezurl}
  {else}
href="{if $current_node.depth|lt(2)}https://clone.digg.one/submit{else}https://clone.digg.one/c#select-a-category-first{/if}"
...
onclick="return false;"
  {else}
  onclick="{if and( $current_node.node_id|ne( 286 ), $current_node.class_identifier|eq('category'), $current_node.node_id|ne(285), $current_node.depth|gt(2))}document.getElementById('btn-submit').click(); return false;{else}{/if}"
  {/if}
  title="{$item.data_map.location.data_text|wash}"
  {if $item_class}class="{$item_class|implode(" ")}"{/if}
  rel={$item.url_alias|ezurl}>submit story!</a>
{/run-once}
...
<li class="dropdown">
  <div class="dropdown-button">
    <a href={$item.url_alias|ezurl} class="dropdown-link" role="button">{$item.name|wash()}</a>
    {if $item.children|count}<button type="button" class="dropdown-toggle" aria-label="dropdown toggler button"></button>{/if}
  </div>
{/if *}

{if and( $item.class_identifier|ne( 'link' ), $current_user.contentobject.id|ne( 10 ) )}
<li class="dropdown">
  <div class="dropdown-button">
    <a href={$item.url_alias|ezurl} class="dropdown-link {if array( 374, 286, 97, 381 )|contains( $item.node_id )}dropdown-folder{/if} {*$item_class|implode(" ")*}" role="button">{$item.name|wash()}</a>
    {if $item.children|count}<button type="button" class="dropdown-toggle" aria-label="dropdown toggler button"></button>{/if}
  </div>
{elseif and( $item.class_identifier|ne( 'link' ), $current_user.contentobject.id|eq( 10 ) )}
<li class="dropdown">
  <div class="dropdown-button">
    <a href={$item.url_alias|ezurl} class="dropdown-link {if array( 374, 286, 97, 381 )|contains( $item.node_id )}dropdown-folder{/if} {*$item_class|implode(" ")*}" role="button">{$item.name|wash()}</a>
    {if $item.children|count}<button type="button" class="dropdown-toggle" aria-label="dropdown toggler button"></button>{/if}
