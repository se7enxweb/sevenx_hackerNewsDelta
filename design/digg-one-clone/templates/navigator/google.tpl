...
{*       	<span class="current">1</span> *}
       <span class="previous"><a href={concat($page_uri,$:item_previous|gt(0)|choose('',concat($:offset_text,$:item_previous)),$:view_parameter_text,$page_uri_suffix)|ezurl}><span class="text">&laquo;&nbsp;{* "Prev"|i18n("design/standard/navigator") *}</span></a></span>
       {/case}
       {case match=1}
       {/case}
     {/switch}
...
{/if}
    ...
    {section loop=$:right_length}
        {let page_offset=sum($ViewParameter:current_page,1,$:index)}
          <span class="other last"><a href={concat($page_uri,$:page_offset|gt(0)|choose('',concat($:offset_text,mul($:page_offset,$item_limit))),$ViewParameter:view_parameter_text,$page_uri_suffix)|ezurl}>{$:page_offset|inc}</a></span>
        {/let}
    {/section}
...
*}
    ...
    {switch match=and($:item_next|le( $item_count ) )}
      {case match=1}
        <span class="nextprev"><a href={concat($page_uri,$:offset_text,$:item_next,$:view_parameter_text,$page_uri_suffix)|ezurl}><span class="text">{* "Next"|i18n("design/standard/navigator") *}{* $:item_next}@{$item_count *}&nbsp;&raquo;</span></a></span>
      {/case}
      {case}
      {/case}
    {/switch}
