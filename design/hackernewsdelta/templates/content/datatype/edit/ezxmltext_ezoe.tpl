...
{if $input_handler.is_editor_enabled}
    ...
    <script id="tinymce_script_loader" type="text/javascript" src={"javascript/tiny_mce_jquery.js"|ezdesign}
    ...
    (function() {
        var uri = document.location.protocol + '//' + document.location.host + eZOeGlobalSettings.ez_tinymce_url,
            tps = eZOeGlobalSettings.plugins.split(','),
            pm = tinymce.PluginManager,
            tp;
...
<div class="oe-window">
    <textarea class="box" id="{$attribute_base}_data_text_{$attribute.id}"
        name="{$attribute_base}_data_text_{$attribute.id}" cols="88"
        rows="{$editorRow}">{$input_handler.input_xml}</textarea>
</div>

<div class="form-group block">
    <div class="btn-group" style="padding-top: 0.5rem;">
        {if $input_handler.can_disable}
        <input class="btn {if $layout_settings['buttons']|contains('disable')} hide{/if}" type="submit"
            name="CustomActionButton[{$attribute.id}_disable_editor]"
            value="{'Disable editor'|i18n('design/standard/content/datatype')}" />
        {/if}
    </div>
    <script type="text/javascript">
...
<input class="btn" type="submit" name="CustomActionButton[{$attribute.id}_enable_editor]"
    value="{'Enable editor'|i18n('design/standard/content/datatype')}" /><br />
    {/let}
{/if}
{/default}