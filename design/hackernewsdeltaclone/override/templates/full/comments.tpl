                    ...
                    {if gt( $node.depth, '1')}
                      {if $children_count}
                        {foreach $childrenNodes as $index => $child}
                          ...
                          {if and( is_object($child[1]), $total_index|ge(0), $counter|le($page_limit_navigator) )}
                            {node_view_gui view=line content_node=$child[1] current_node=$node index=$total_index indent=$child[1].depth|sub(6)}
                          {/if}
                          {/cache-block}
