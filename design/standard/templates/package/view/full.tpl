        ...
        <div class="maintainers">
            ...
            <p>
                {section var=maintainer loop=$package.maintainers}
                    <a href="mailto:{$maintainer.item.email}" subject="{"Regarding Exponential package '%packagename'"|i18n('design/standard/package',,hash( '%packagename', $package.name ) )}" title="{'Send email to the maintainer'|i18n('design/standard/package')}">{$maintainer.item.name|wash}</a> ({$maintainer.item.role|wash})
                {delimiter}, {/delimiter}
                {/section}
            </p>
        </div>
        ...
        <div class="changelog">
            ...
            <p>
                {section var=log loop=$package.changelog}
                    <h3><p class="byline"><em class="date">({$log.item.timestamp|l10n( shortdatetime )})</em> <a href="mailto:{$log.item.email}" subject="{"Regarding Exponential package '%packagename'"|i18n('design/standard/package',,hash( '%packagename', $package.name ) )}" title="{'Send email to the maintainer'|i18n('design/standard/package')}">{$log.item.person|wash}</a></p></h3>
                    <ul>
