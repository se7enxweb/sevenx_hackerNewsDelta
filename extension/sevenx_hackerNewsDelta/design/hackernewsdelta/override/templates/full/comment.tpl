...
<div>
                  <h2 class="article-title">
                    <a href={$node.data_map.link.content|ezurl}>{$node.data_map.subject.content|wash}</a>
                    {* <a href={$node.data_map.link.content|ezurl} class="article-source">({$node.data_map.link.content|explode('https://').1|explode('/')[0]})</a> *}
                  </h2>
                  <div class="article-meta">
                    <span class="article-score">{if $total_diggs|gt(0)}{$total_diggs}{else}0{/if} points</span> by
                    {def $user_submited=$node.object.author_array.0 $user_submited_username=$user_submited.login $user_submited_node=$user_submited.contentobject.main_node.url_alias}<a class="link article-author" href={$user_submited_node|ezurl}>{$user_submited_username}</a>
                    ...
                    <span class="article-time">{if $hours_ago|lt(24)}{$hours_ago} hours{else}{$days_ago} days{/if} ago on {$node.object.published|datetime( 'custom', '%Y/%m/%d @ %h:%i %a' )}</span>
		    <br>
                    ...
                    <a href={concat( $node.url_alias, '#comments' )|ezurl} class="link">{$node.children|count} comments</a>
                  </div>
                </div>
              </article>
            </li>
     ...
     {section show=$node.data_map.message.content.is_empty|not}<div class="">{attribute_view_gui attribute=$node.data_map.message}</div>{/section}
        ...
        <div class="border-rounded" style="padding-top:1rem;">
            <h2 id="comments">{"Comments"|i18n("design/ezwebin/full/article")}</h2>
                ...
                {if fetch( 'content', 'access', hash( 'access', 'create',
                    ...
                    <input class="btn btn-primary button new_comment" type="submit" name="NewButton" value="{'New comment'|i18n( 'design/ezwebin/full/article' )}" />
                    </form>
                {else}
