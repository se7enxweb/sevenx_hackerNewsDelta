...
{foreach $content_attributes_grouped_data_map as $attribute_group => $content_attributes_grouped}
    {*if $attribute_group|ne( $attribute_default_category )}
        <fieldset class="ezcca-collapsible form-fieldset">
        <legend class="form-legend"><a href="JavaScript:void(0);">{$attribute_categorys[$attribute_group]}</a></legend>
    ...
    {foreach $content_attributes_grouped as $attribute_identifier => $attribute}
        ...
        <div {if array('diggs','enable_comments','star_rating', 'moderation')|contains($attribute.contentclass_attribute_identifier)}
            style="display:none" {/if}
            class="block ezcca-edit-datatype-{$attribute.data_type_string} ezcca-edit-{$attribute_identifier} form-group">
            ...
            {if and( eq( $attribute.can_translate, 0 ), ne( $object.initial_language_code, $attribute.language_code ) )}
                <label
                    class="form-label">{first_set( $contentclass_attribute.nameList[$content_language], $contentclass_attribute.name )|wash}
                    ...
                    {if $contentclass_attribute.description}
                        <span
                            class="classattribute-description">{first_set( $contentclass_attribute.descriptionList[$content_language], $contentclass_attribute.description)|wash}</span>
                    {/if}
                </label>
            ...
            {else}
                {if $is_translating_content}
                    <label class="form-label {if $attribute.has_validation_error} 'message-error' {/if}">
                        {first_set( $contentclass_attribute.nameList[$content_language], $contentclass_attribute.name )|wash}
                        {if $attribute.is_required}
                            <span class="required">({'required'|i18n( 'design/admin/content/edit_attribute' )})</span>
                        {/if}
                ...
                {else}
                    {if $attribute.display_info.edit.grouped_input}
                        <fieldset class="form-fieldset">
                            <legend class="form-legend {if $attribute.has_validation_error} 'message-error' {/if}">
                                {first_set( $contentclass_attribute.nameList[$content_language], $contentclass_attribute.name )|wash}
                                {if $attribute.is_required}
                                    <span class="required">({'required'|i18n( 'design/admin/content/edit_attribute' )})</span>
                                {/if}
    ...
    <script type="text/javascript">
        {literal}
            jQuery(function($) {
                ...
                // Collapse by default
                $('fieldset.ezcca-collapsible').addClass('ezcca-collapsed').find(
                    'div.ezcca-collapsible-fieldset-content').hide();
            });
        {/literal}
    </script>
    {* /if *}
