...
<h1 class="context-title">{'Votes for story, "<%object_name>" (%collection_count)'|i18n( 'design/admin/infocollector/collectionlist',, hash( '%object_name', $object.name, '%collection_count', $collection_count ) )|wash}</h1>
...
{*    <th class="tight"><img src={'toggle-button-16x16.gif'|ezimage} width="16" height="16" alt="{'Invert selection.'|i18n( 'design/admin/infocollector/collectionlist' )}" title="{'Invert selection.'|i18n( 'design/admin/infocollector/collectionlist' )}" onclick="ezjs_toggleCheckboxes( document.collections, 'CollectionIDArray[]' ); return false;" /></th>
    ...
    <th>{'Vote'|i18n( 'design/admin/infocollector/collectionlist' )}</th>
...
{*    <td><input type="checkbox" name="CollectionIDArray[]" value="{$Collections.item.id}" title="{'Select collected information for removal.'|i18n( 'design/admin/infocollector/collectionlist' )}" /></td>
    ...
    <td>
        {if $Collections.attributes.0.data_int|eq(0)}
	{first_set( $Collections.item.attributes[ 0 ].contentobject_attribute.content.option_list[ 0 ].value, 'digg' )|explode('digg')|implode('vote')|upfirst}
        {else}
        {first_set( $Collections.item.attributes[ 0 ].contentobject_attribute.content.option_list[ 1 ].value, 'undigg' )|explode('digg')|implode('vote')|upfirst}
	{/if}</td>
...
{include name=navigator
         ...
         page_uri=concat( '/spy/collectionlist/', $object.id )
