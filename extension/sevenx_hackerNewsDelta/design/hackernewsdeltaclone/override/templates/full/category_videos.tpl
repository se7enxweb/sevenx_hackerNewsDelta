...
{def $fetch_limit = 260
     $page_limit = 10
     $classes = array( 'link','category','article', 'image', 'user', 'youtube_video' )
     $children = array()
     $children_count = ''
     $childNodes = array()}

{if le( $node.depth, '3')}
  {set $classes=array( 'link','category','article','image', 'user' )}
{/if}

{if and( $node.node_id|eq( 12 ), le( $node.depth, '3') )}
  {set $classes=array( 'link', 'article', 'image', 'user', 'youtube_video' )}
{/if}
...
<div class="content-view-full">
	...
	{if eq( ezini( 'folder', 'SummaryInFullView', 'content.ini' ), 'enabled' )}
    	...
    	<div class="content-view-children float-break">
    	    ...
    	    {foreach $children as $index => $child}
                {if and($node.node_id|eq( 862 ), $index|eq( 0 ) )}
			 ...
			 <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
	 		 ...
	 		 <input type="submit" name="NewButton" value="Upload YouTube Video Story" id="btn-submit" style="width:70%"/>
         	    </form>
		{/if}
