...
{if $class_content.selection_type|ne( 0 )} {* If current selection mode is not 'browse'. *}
        ...
        {section show = and( is_set( $class_content.default_placement.node_id ), ne( 0, $class_content.default_placement.node_id ), ne( '', $class_content.object_class ) )}
            ...
            {if and( is_set( $defaultNode ), $defaultNode.can_create )}
                ...
                <input class="btn" type="button" value="Create New" name="CustomActionButton[{$attribute.id}_new_object]"
...
{else}    {* Standard mode is browsing *}
    <div class="form-group block" id="ezobjectrelationlist_browse_{$attribute.id}">
    ...
    {if eq( ezini( 'BackwardCompatibilitySettings', 'AdvancedObjectRelationList' ), 'enabled' )}
            ...
            {if $attribute.content.relation_list}
                <table class="table table-bordered list" cellspacing="0">
                <tr class="bglight">
                ...
                {section name=Relation loop=$attribute.content.relation_list sequence=array( bglight, bgdark )}
                    ...
                    {if $:item.is_modified}
                        ...
                        {* Order. *}
                        <td><input id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_order" class="form-control ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" size="2" type="text" name="{$attribute_base}_priority[{$attribute.id}][]" value="{$:item.priority}" /></td>
                    {else}
                        {let object=fetch( content, object, hash( object_id, $:item.contentobject_id, object_version, $:item.contentobject_version ) )}
                        {* Remove. *}
                        <td>
                        ...
                        {* Order. *}
                        <td><input size="2" type="text" name="{$attribute_base}_priority[{$attribute.id}][]" value="{$:item.priority}" class="form-control" /></td>
                        {/let}
                    {/if}
                    </tr>
                {/section}
            ...
            {else}
                <p>{'There are no images.'|i18n( 'design/standard/content/datatype' )}</p>
            {/if}

            {if $attribute.content.relation_list}
                <input class="buttbtnon" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]" value="{'Remove selected'|i18n( 'design/standard/content/datatype' )}" />&nbsp;
                <input class="btn" type="submit" name="CustomActionButton[{$attribute.id}_edit_objects]" value="{'Edit selected'|i18n( 'design/standard/content/datatype' )}" />
            {else}
                <input class="btn button-disabled" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]" value="{'Remove selected'|i18n( 'design/standard/content/datatype' )}" disabled="disabled" />&nbsp;
                <input class="btn button-disabled" type="submit" name="CustomActionButton[{$attribute.id}_edit_objects]" value="{'Edit selected'|i18n( 'design/standard/content/datatype' )}" disabled="disabled" />
            {/if}

            {if array( 0, 2 )|contains( $class_content.type )}
                <input class="btn" type="submit" name="CustomActionButton[{$attribute.id}_browse_objects]" value="{'Add objects'|i18n( 'design/standard/content/datatype' )}" />
            ...
            {section show=and( $can_create, array( 0, 1 )|contains( $class_content.type ) )}
                ...
                {/if} 
                <input class="btn" type="submit" name="CustomActionButton[{$attribute.id}_new_class]" value="{'Create new object'|i18n( 'design/standard/content/datatype' )}" />
                </div>
            {/section}
    ...
    {else}
        ...
        <table class="table table-bordered list{if $attribute.content.relation_list|not} hide{/if}" cellspacing="0">
        <thead>
        <tr>
            <th class="tight"><img src={'toggle-button-16x16.gif'|ezimage} alt="{'Invert selection.'|i18n( 'design/standard/content/datatype' )}" onclick="ezjs_toggleCheckboxes( document.editform, '{$attribute_base}_selection[{$attribute.id}][]' ); return false;" title="{'Invert selection.'|i18n( 'design/standard/content/datatype' )}" /></th>
            <th>{'Name'|i18n( 'design/standard/content/datatype' )}</th>
        ...
        {else}
          <tr class="bgdark">
            ...
            <td>--section-name--</td>
            <td>--published--</td>
            <td><input size="2" type="text" name="{$attribute_base}_priority[{$attribute.id}][]" value="0" class="form-control" /></td>
          </tr>
        {/if}
        </tbody>
        </table>
        {if $attribute.content.relation_list|not}
            <p class="ezobject-relation-no-relation" style="margin-block: 0.5rem;">{'There are no images.'|i18n( 'design/standard/content/datatype' )}</p>
        {/if}

        <div class="block inline-block" style="margin-top: 1rem;">
	        {if $attribute.content.relation_list}
	            <input class="btn ezobject-relation-remove-button" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]" value="{'Remove selected'|i18n( 'design/standard/content/datatype' )}" title="{'Remove selected elements from the relation'|i18n( 'design/standard/content/datatype' )}" />
	        {else}
	            <input class="btn button-disabled ezobject-relation-remove-button" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]" value="{'Remove selected'|i18n( 'design/standard/content/datatype' )}" disabled="disabled" />
	        {/if}
        </div>
        {* <h4 style="margin-block: 0.5rem;">{'Add image(s)'|i18n( 'design/standard/content/datatype' )}</h4> *}
        <div class="left" style="margin-top: 1rem;">
	        ...
	        <input class="btn" type="submit" name="CustomActionButton[{$attribute.id}_browse_objects]" value="{'Add image(s))'|i18n( 'design/standard/content/datatype' )}" title="{'Browse to add existing objects in this relation'|i18n( 'design/standard/content/datatype' )}" />
        ...
        <div class="btn-group right" style="margin-top: 1rem;">
            ...
            <input type="submit" class="btn hide ezobject-relation-search-btn" name="CustomActionButton[{$attribute.id}_browse_objects]" value="{'Find objects'|i18n( 'design/standard/content/datatype' )}" />
        </div>
        <div class="break"></div>
