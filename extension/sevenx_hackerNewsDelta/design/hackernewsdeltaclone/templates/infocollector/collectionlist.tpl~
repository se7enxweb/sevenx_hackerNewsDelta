...
<h1 class="context-title">{'Diggs for story, "<%object_name>" (%collection_count)'|i18n( 'design/admin/infocollector/collectionlist',, hash( '%object_name', $object.name, '%collection_count', $collection_count ) )|wash}</h1>
...
{*    <td><input type="checkbox" name="CollectionIDArray[]" value="{$Collections.item.id}" title="{'Select collected information for removal.'|i18n( 'design/admin/infocollector/collectionlist' )}" /></td>
	...
	{$Collections.item.attributes[ 0 ].contentobject_attribute.content.option_list[ 0 ].value}
        {else}
        {$Collections.item.attributes[ 0 ].contentobject_attribute.content.option_list[ 1 ].value}
	{/if}</td>
